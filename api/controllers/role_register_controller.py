...
def user_role_register():
    try:
        ...
        if (
            data.get("role") != "musician"
            and data.get("role") != "manager"
            and data.get("role") != "technician"
        ):
            return jsonify({"status": 400, "message": "Invalid role provided"}), 400

        if data.get("role") == "musician":
            if (
                Musician.query.filter_by(user_id=current_user.get("id")).first()
                is not None
            ):
                return (
                    jsonify({"status": 400, "message": "Musician role already exists"}),
                    400,
                )
            musician = Musician()
            ...
            return (
                jsonify(
                    {
                        "message": "Musician role added successfully",
                        "token": generate_newToken(current_user),
                    }
                ),
                200,
            )
        ...
        if data.get("role") == "manager":
            if (
                Venue_Manager.query.filter_by(user_id=current_user.get("id")).first()
                is not None
            ):
                return (
                    jsonify({"status": 400, "message": "Manager role already exists"}),
                    400,
                )
            ...
            return (
                jsonify(
                    {
                        "status": "success",
                        "message": "Manager role added successfully",
                        "token": generate_newToken(current_user),
                    }
                ),
                200,
        ...
        if data.get("role") == "technician":
            if (
                Technician.query.filter_by(user_id=current_user.get("id")).first()
                is not None
            ):
                return (
                    jsonify(
                        {"status": 400, "message": "Technician role already exists"}
                    ),
                    400,
                )
            ...
            return (
                jsonify(
                    {
                        "status": "success",
                        "message": "Technician role added successfully",
                        "token": generate_newToken(current_user),
                    }
                ),
                200,
