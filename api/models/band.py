...
class Band(db.Model):
    ...
    memebrs_not_registred = db.relationship("Musician_Not_Registred", backref="bands", lazy=True, secondary="band_members")
    ...
    def serialize(self):
        return {
            ...
            "members_not_registred": [member.serialize() for member in self.memebrs_not_registred],
            "all_members": [member.serialize() for member in self.members] + [member.serialize() for member in self.memebrs_not_registred],
