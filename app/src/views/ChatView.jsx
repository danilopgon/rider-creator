...
export const ChatView = () => {
  ...
  const [listMessagesByConversation, setListMessagesByConversation] =
    useState(null);
  ...
  useEffect(() => {
    ...
    socket.on("message", (data) => {
      ...
      const msg = async () => {
        await setListMessagesByConversation(
          getMessages(selectConversation?.id)
        );
      };
      msg();
    });
    ...
    socket.on("chat", (data) => {
      if (data.msg === "ya existe esta conversacion") {
        ...
        const conversation = listChatByMyUser.find(
          (chat) => String(chat.id) === String(data?.data?.id)
        );
  ...
  const handleCreateChat = (e) => {
    try {
      ...
      const to_user = listFindUser.find(
  ...
  }, [selectConversation, listChatByMyUser]);
  ...
  return (
    <section className="flex flex-col-reverse md:flex-row w-full h-full">
